...
function Login() {
	...
	const { loading, isAuth, loginSuccess } = useAuth();
	const navigate = useNavigate();

	// If already logged in, redirect immediately
	if (!loading && isAuth) {
		navigate('/inventorydata', { replace: true });
		return null;
	}
	...
	const handleSubmit = async (e) => {
		...
		try {
			// POST to login using apiFetch (includes CSRF & credentials automatically)
			...
			if (response.ok) {
				...
				// console.log('Login successful!', message, firstName, lastName, userId, role);
